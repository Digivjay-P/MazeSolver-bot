...
//----------FOLLOW-------------------
        ...
        if(prev_state == S_FORWARD_BEFORE)begin
            if((R_diff > swf_before) || obst_f )begin
                ...
                if(cmd_in == 2'd0)begin   //left turn 
                    sswf <= 1'b0;         //follow right wall
                end
                else if(cmd_in == 2'd1)begin
                    sswf <= 1'b1;    //follow left wall
                end
            end
        end
            ...
            else if(event_out == 2'd0)begin  //left turn was detected but not taken follow right wall
                sswf <= 1'b0; //follow right wall
                done_out <= 1'b0;
            end
            else if(event_out == 2'd1)begin  //right was detected but not taken 
                sswf <= 1'b1; //follow left wall 
                done_out <= 1'b0;
            end
        end
    ...
    S_TURN: begin
	 in_follow <= 1'b0;
        ...
        else if(cmd_in == 2'd2)begin  //u turn
...
S_SINGLE_WALL_TRACK : begin
	IN1 = 1;  IN2 = 0;
	IN3 = 1;  IN4 = 0;
	if(sswf == 1'b0)begin  //left turn so right wall follow
