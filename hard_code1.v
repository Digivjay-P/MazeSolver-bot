...
(* keep *)	 reg [2:0] state;          // Current State
	...
	localparam turn_R= 1167;
	localparam turn_L = 1582;  //earlier 1577
         ...
         turn_left_mem <= 0;
            ...
            if (state_timer > 0) begin
				...
				case(state)
					...
					S_FOLLOW : begin  
								//state_timer <= 00 ;
							if ((!obst_l && obst_r) && !(turn_count >= 3 && turn_count < 5)) begin   //obstacle in right 
                          turn_left_mem <= 2'd1; // Turn Left  
								  state <= S_FORWARD_BEFORE; 
								  state_timer <= FORWARD_TIMEOUT;
								end
							else if((obst_l && !obst_r)) begin  //obstacle in left 
